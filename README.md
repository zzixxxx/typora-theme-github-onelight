...
└─ style/
   ...
   ├─ text.css               粗体 / 下划线 / 删除线 / 高亮 / 链接图标 / kbd / 选区 / 复选框（mdmdt，含表格内复选框）
...
| `02-表格.md` | 基础表格、对齐、表格内行内元素、表格内复选框、宽表格 |
...
- 关闭：点击遮罩任意位置。打开期间点不到其他图片，要先关闭再打开下一张。点击放大图本身不做任何事。
- 只有指针悬停在图片上时才会放大。点击图片旁边的空白、用键盘把光标移进图片，Typora 会像原生一样显示这张图的 `![alt](src)` 源码，但不会放大。
- 不显示 zoom-in / zoom-out 光标：放大态尺寸固定，避免误以为可以逐级缩放。
- 原理：利用 Typora 点击图片时给 `.md-image` 加 `.md-expand` 的机制，把 img 改为 `position: fixed` 居中；放大后由相邻文字段落的透明全屏"点击捕获层"维持悬停状态，点击它时光标落到该段开头，Typora 重算 `.md-expand`，灯箱关闭。前后相邻块都不是文字段落 / 标题时没有捕获层，指针离开放大图即恢复原状。
- 已兼容 `偏好设置 → 段落首行缩进`。
